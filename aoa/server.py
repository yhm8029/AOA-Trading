...
from .review import ReviewStore as Store, missing_plan
    ...
    def start_job(self,fn,cleanup=lambda:None):
        ...
        def run():
            try:
                report=fn(progress)
                with self.job_lock:
                    self.job={'id':job_id,'state':'done','report':report,'message':'완료','progress':100}
            except JobCancelled as exc:
                with self.job_lock:
                    self.job={'id':job_id,'state':'cancelled','message':str(exc)}
            except Exception as exc:
                with self.job_lock:
                    self.job={'id':job_id,'state':'error','message':str(exc)[:600]}
            finally:
                cleanup()
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        try:
            if path=='/api/status':
                return self.send_json(200,{'token':self.server.token,**self.server.store.status(),'version':VERSION})
            if path=='/api/job':
                with self.server.job_lock: snapshot=dict(self.server.job)
                return self.send_json(200,snapshot)
            if path=='/api/episodes':
                return self.send_json(200,self.server.store.episodes(q.get('symbol','XBTUSD'),q.get('year',''),q.get('direction',''),q.get('result',''),q.get('search',''),q.get('year_mode','entry')))
            if path=='/api/events':
                return self.send_json(200,self.server.store.events(q.get('episode','')))
            if path=='/api/performance':
                return self.send_json(200,self.server.store.performance(q.get('episode','')))
            if path=='/api/coverage':
                return self.send_json(200,missing_plan(self.server.store,q.get('pair','BTCUSDT'),int(q['start']),int(q['end'])))
            if path=='/api/candles':
    ...
    def do_POST(self):
        ...
        try:
            ...
            if path=='/api/cancel-job':
                with self.server.job_lock:
                    if body.get('job_id') and body['job_id']!=self.server.job.get('id'):
                        raise ValueError('작업이 이미 변경됐습니다.')
