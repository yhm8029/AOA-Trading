...
from .model import adapt_order, validate_bar, time_us
...
class BoundedReader(io.RawIOBase):
    """Bound expanded CSV bytes across an entire input, including gzip members."""
    def __init__(self, source, budget):
        self.source=source; self.budget=budget
    def readable(self):
        return True
    def readinto(self, buffer):
        data=self.source.read(len(buffer))
        self.budget[0]+=len(data)
        if self.budget[0]>MAX_ARCHIVE_BYTES:
            raise ValueError('압축 해제된 CSV 크기 안전 한도 초과')
...
def import_file(store,path,display_name=None,progress=lambda x:None):
    path=Path(path);digest=sha256(path);name=display_name or path.name
    ...
    with store.connect() as db:
        ...
        def parse_text(text,source):
            reader=csv.reader(text);first=next(reader,None)
            if not first:
                return
            ...
            if raw:
                ...
                headers=[str(x).strip() for x in first];kind=csv_kind(headers)
                if kind=='unknown':
            ...
            for line,row in iterable:
                ...
                try:
                    if kind=='orders':
                        # Extractor v1 deliberately calls group quantity 'audit only'.
                        if 'qty' not in row and row.get('order_group_qty_audit_only'):
                            row['qty']=row['order_group_qty_audit_only']
                        item=adapt_order(row)
                        Store.order(db,item,source)
                        counts['order_endpoints']+=1
                    else:
                        if kind=='binance':
                            if len(row)<12: raise ValueError('Binance kline columns < 12')
                            ...
                            if stamp:
                                us=time_us(stamp)
                                if us%60_000_000: raise ValueError('Normalized 1m candle must open on minute boundary')
                                t=us//1_000_000
                        ...
                        if status=='conflict':problem(source,line,'충돌 캔들 격리: '+str(bar[:2]))
        ...
        def walk(stream,source,depth=0):
            if source.lower().endswith('.zip'):
                if depth>2:raise ValueError('ZIP nesting depth exceeded')
                with zipfile.ZipFile(stream) as z:
                    entries=safe_members(z);entries.sort(key=lambda i:('features' in i.filename,i.filename))
                    for info in entries:
                        if info.is_dir():continue
                        suffix=info.filename.lower();child=source+'/'+info.filename
                        if suffix.endswith(('.csv','.csv.gz')):
                            with z.open(info) as raw:
                                if suffix.endswith('.gz'):
                                    with gzip.GzipFile(fileobj=raw) as gz:parse(gz,child)
                                else:parse(raw,child)
                        elif suffix.endswith('.zip') and re.search(r'[A-Z0-9]+-1m-\d{4}-\d{2}',info.filename):
                            if info.file_size>64*1024**2:raise ValueError('nested monthly ZIP >64MB')
                            with z.open(info) as raw:walk(io.BytesIO(raw.read()),child,depth+1)
                        else:skipped.append(child)
            elif source.lower().endswith('.gz'):
                with gzip.GzipFile(fileobj=stream) as gz:parse(gz,source)
        ...
        try:
            ...
            report={'name':name,'sha256':digest,'counts':dict(counts),'skipped_members':skipped[:120],
                    'expanded_csv_bytes':budget[0],'note':'원본 미수정. CSV 사전 특징만 저장. post_*는 예측 입력에 적재하지 않음.'}
