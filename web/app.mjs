import {FRAMES,COLORS,category,label,color,qty,px,bucket,eligible,groupMarkers,priceReturn,formatTime,desiredRange,safeCsv} from './core.mjs';
const $=id=>document.getElementById(id);
const S={token:'',episodes:[],filtered:[],ep:null,events:[],selected:null,tf:5,tz:'UTC',range:null,bars:[],groups:[],noteVersion:0,dirty:false,request:0,chartAbort:null,categories:new Set(['ENTRY','ADD','FLIP','INCREASE','TP','CUT','STOP','CLOSE','NEAR_CLOSE','REDUCE']),theme:localStorage.getItem('aoa-theme')||'light'};
...
async function loadEpisode(e){
 ...
 $('event-detail').replaceChildren(el('p','주문을 선택하세요.'));$('note-status').textContent='';
 renderEpisodes();
...
async function loadChart(range){
 if(!S.ep)return;
 if(!S.ep.pair){$('empty-state').hidden=false;toast('해당 계약의 시장 매핑이 확인되지 않았습니다.',true);return;}
 S.chartAbort?.abort();const controller=new AbortController();S.chartAbort=controller;const generation=S.request;
 $('loading').hidden=false;
 try{const meta=await api('/api/chart?'+new URLSearchParams({pair:S.ep.pair,tf:S.tf,start:Math.floor(range.start),end:Math.ceil(range.end)}),{signal:controller.signal});if(generation!==S.request)return;S.range={start:meta.start,end:meta.end};S.bars=meta.bars;S.chartMeta=meta;markers.setMarkers([]);candles.setData(meta.bars.map(b=>b.open===undefined?{time:b.time}:{time:b.time,open:b.open,high:b.high,low:b.low,close:b.close}));volume.setData(meta.bars.map(b=>b.open===undefined?{time:b.time}:{time:b.time,value:b.volume,color:b.close>=b.open?'rgba(8,153,129,0.48)':'rgba(242,54,69,0.48)'}));chart.timeScale().fitContent();$('empty-state').hidden=meta.valid>0;if(!meta.valid){$('empty-state').querySelector('h3').textContent='이 구간의 실제 캔들이 없습니다.';$('empty-state').querySelector('p').textContent='캔들 ZIP을 가져오거나, 위의 공개 분봉 보완 버튼을 사용하세요.';}drawEvents();showEpisodeStats();syncPriceLine();$('jump-date').value=date((S.selected?S.selected.time_us:((meta.start+meta.end)/2)*1e6),false).replace(' ','T');if(range.windowed)toast('긴 포지션입니다. 일부 구간부터 표시합니다. 주문 클릭 또는 전체 포지션 버튼으로 이동하세요.');}catch(err){if(err.name!=='AbortError')toast(err.message,true);}finally{if(S.chartAbort===controller)$('loading').hidden=true;}
}
...
function syncPriceLine(){if(priceLine){candles.removePriceLine(priceLine);priceLine=null;}if($('show-price-line').checked&&S.selected?.first_price){priceLine=candles.createPriceLine({price:S.selected.first_price,color:color(S.selected),lineWidth:1,lineStyle:2,axisLabelVisible:true,title:'BitMEX 첫 체결 (대체차트 참고)'});}}
...
function download(name,blob){const link=document.createElement('a');const url=URL.createObjectURL(blob);link.href=url;link.download=name;link.click();setTimeout(()=>URL.revokeObjectURL(url),60000);}
function snapshot(){if(!S.chartMeta?.valid)return toast('캔들이 있는 구간을 먼저 선택하세요.');const image=chart.takeScreenshot();const canvas=document.createElement('canvas');canvas.width=image.width;canvas.height=image.height+92;const ctx=canvas.getContext('2d');ctx.fillStyle=themeColors().background;ctx.fillRect(0,0,canvas.width,canvas.height);ctx.fillStyle=themeColors().text;ctx.font='bold 17px sans-serif';ctx.fillText($('chart-title').textContent,15,24);ctx.font='11px sans-serif';ctx.fillText('Binance spot candles / BitMEX event times | Retrospective trade study | '+S.tz,15,44);ctx.drawImage(image,0,56);ctx.fillText('Charts by TradingView - AOA Whale Viewer. Gaps are missing data, not interpolated prices.',15,canvas.height-12);canvas.toBlob(b=>download(`AOA_${S.ep.symbol}_${S.ep.episode_id}_${S.tf}m.png`,b));}
function exportEvents(){if(!S.ep)return;const header=['episode_id','symbol','direction','event','time_utc','first_price','group_vwap','group_qty','qty_before','qty_after','source_orderid','classification_reason'];const rows=visibleEvents().map(e=>[e.episode_id,e.symbol,e.direction,e.event,e.event_time_utc,e.first_price,e.group_vwap,e.qty,e.qty_before,e.qty_after,e.source_orderid,e.classification_reason]);download(`AOA_${S.ep.symbol}_${S.ep.episode_id}_events.csv`,new Blob(['\uFEFF'+[header,...rows].map(r=>r.map(safeCsv).join(',')).join('\r\n')],{type:'text/csv;charset=utf-8'}));}
function bindings(){
