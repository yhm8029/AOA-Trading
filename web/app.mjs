...
boot().catch(error);
window.AOAViewer={snapshot:()=>({episode:S.episode?.id,tf:S.tf,barCount:S.bars.filter(isBar).length,visibleBarCount:S.visible.filter(isBar).length,markerCount:S.markerGroups.length,cutoff:S.cutoff,anchor:S.anchor,start:S.start,end:S.end,replayIndex:S.replayIndex,playing:S.playing,hover:S.hover,coverage:S.coverage,performance:S.cutoff==null?S.performance:null,selected:S.selected,study:S.study,loading:S.loading,version:UI_VERSION}),pointForBar:t=>{const b=S.visible.find(x=>x.time===t);if(!isBar(b))return null;return{x:chart.timeScale().timeToCoordinate(t),y:candles.priceToCoordinate((b.open+b.close)/2)};}};
