...
let chart,candles,volume,markerApi,priceLine=null,basisLine=null,aborter=null;
let toastTimer;
...
function initChart(){
  ...
  chart.subscribeCrosshairMove(p=>{
    if(!p.time)return;
    const b=S.bars.find(x=>x.time===p.time);if(!b||b.open==null)return;
    const rv=relativeVolume(S.bars,S.bars.indexOf(b));
  ...
  chart.subscribeClick(p=>{
    if(!p.time)return;
    const hits=S.events.filter(e=>bucket(S.endpoint==='last'?e.end_time:e.time,S.tf)===p.time && (S.cutoff==null||e.first_us<=S.cutoff*1e6));
    if(hits.length){const index=hits.findIndex(e=>e.id===S.selected);selectEvent(hits[(index+1)%hits.length],false);}
  });
...
function renderChart(fit=false){
  const bars=completeAt(S.bars,S.tf,S.cutoff);
...
function renderEpisodes(){
  ...
  for(const ep of S.episodes){const el=node('button',null,'episode-item'+(S.episode?.id===ep.id?' selected':''));el.dataset.episode=ep.id;
    const row=node('div',null,'row');row.append(node('strong','#'+ep.id),node('span',ep.direction==='Short'?'SHORT':ep.direction==='Long'?'LONG':'방향 미확보','badge '+ep.direction.toLowerCase()));el.append(row,node('small',date(ep.start)+' · '+ep.count+'주문'));
...
function renderStats(){
  ...
  const ep=S.episode;const vals=S.cutoff!=null?[['방향',ep.direction],['복기 모드','성과 숨김']]:[['방향',ep.direction],['표시 주문',String(S.events.length)+'건'],['관측 최대 보유',fmtQty(ep.max_observed_qty)],['원장 순손익',ep.pnl_btc==null?'미확보':ep.pnl_btc.toFixed(3)+' BTC']];
...
function resetReplay(){clearInterval(S.timer);S.timer=null;S.cutoff=null;$('replayEnabled').checked=false;$('play').textContent='▶ 재생';$('replayTime').textContent='꺼짐';$('result').disabled=false;$('minQty').disabled=false;$('legacy').disabled=false;$('endpoint').disabled=false;}
async function selectEpisode(ep){
  resetReplay();S.episode=ep;S.selected=null;S.events=[];S.bars=[];renderChart();renderEpisodes();
  $('detailTitle').textContent='포지션 #'+ep.id;$('chartTitle').textContent=`${ep.pair||'시장 미확보'} · ${S.tf} · #${ep.id} ${ep.direction}`;
  const targetId=ep.id;const events=await api('events?episode='+encodeURIComponent(ep.id));if(S.episode.id!==targetId)return;S.events=events;
  ...
  const f=ep.focus;await loadWindow(f);
  const initial=events.find(e=>e.time>=f);if(initial)selectEvent(initial,false);
}
async function loadWindow(center,range=null){
  ...
  const step=TF[S.tf];S.start=range?range[0]:bucket(center,S.tf)-180*step;S.end=range?range[1]:bucket(center,S.tf)+360*step;
...
function renderEvents(){
  const events=allowedEvents();$('eventCount').textContent=`${events.length.toLocaleString()} / ${S.events.length.toLocaleString()} 주문`;
  $('eventList').replaceChildren();
  for(const e of events){
    const action=S.cutoff!=null?(e.role==='Entry'?'INCREASE':'REDUCE'):displayAction(e,$('legacy').checked);
    const button=node('button',null,'event-item'+(e.id===S.selected?' selected':''));button.dataset.event=e.id;
    const top=node('div',null,'event-top');top.append(node('strong',`${e.direction==='Short'?'숏':'롱'} ${ACTIONS[action]?.label||action}`),node('span',S.cutoff==null?fmtQty(e.qty):''));
    button.append(top,node('span',date($('endpoint').value==='last'?e.end_time:e.time,true)+(S.cutoff==null?' · $'+money(e.price):''),'event-sub'));
...
function drawDetail(e){
  ...
  target.append(node('h3',`${e.direction} · ${e.role==='Entry'?'진입/추가':'감량/종료'} 주문`));
  target.append(detailRow('첫 체결',date(e.time,true)+' '+S.zone));
  if(S.cutoff!=null){target.append(node('p','복기 중에는 분할체결의 최종 수량·종료시각·사후 분류를 숨깁니다. 분 단위 시장 복기이며 개별 틱 재생은 아닙니다.','warning'));return;}
  const rows=[['원본 시각',e.time_utc],['마지막 체결',e.last_observed?e.end_time_utc:'미확보'],['총 체결수량',Number(e.qty).toLocaleString()+' 계약'],['첫 체결가격',money(e.first_price)],['산술평균가격',money(e.avg_price)],['역수가중가격',money(e.inverse_price)],['시작 전 보유',fmtQty(e.position_before)],['종료 뒤 보유',fmtQty(e.position_after)],['시작 전 평균단가',money(e.basis_before)],['가격 변화율*',e.price_move_pct==null?'미확보':e.price_move_pct.toFixed(3)+'%'],['스톱 발동가격',e.stop_trigger?money(e.stop_trigger):'미기록'],['주문유형',e.order_type||'미확보']];
  for(const row of rows)target.append(detailRow(...row));
  const fields={...e.context,...e.features};
  const volume1=fields.pre_volume_1m_vs_prior_20m;
  const volume5=fields.pre_volume_5m_vs_previous_20m??fields.pre_volume_5m_vs_prior_20m;
...
async function selectEvent(e,jump=true){
  ...
  S.selected=e.id;renderEvents();drawDetail(e);renderMarkers();
  const stamp=$('endpoint').value==='last'?e.end_time:e.time;
  if(jump){
    if(stamp<S.start||stamp>=S.end){resetReplay();await loadWindow(stamp);}
    chart.timeScale().setVisibleRange({from:bucket(stamp,S.tf)-35*TF[S.tf],to:bucket(stamp,S.tf)+65*TF[S.tf]});
    $('gotoDate').value=localDateInput(stamp,S.zone);
  }
  const entry=$('eventList').querySelector(`[data-event="${e.id}"]`);entry?.scrollIntoView({block:'nearest'});
}
function applyReplay(){
  if(!$('replayEnabled').checked){resetReplay();renderChart();renderEvents();renderStats();renderEpisodes();drawDetail(S.events.find(e=>e.id===S.selected));return;}
  if(!S.bars.length)return;
  const i=Math.min(+$('replaySlider').value,S.bars.length-1);S.replayIndex=i;S.cutoff=S.bars[i].time+TF[S.tf];
  $('replayTime').textContent=date(S.cutoff);$('result').disabled=true;$('minQty').disabled=true;$('legacy').disabled=true;$('endpoint').disabled=true;
  if(S.events.find(e=>e.id===S.selected)?.first_us>S.cutoff*1e6)S.selected=null;
  renderChart();renderEvents();renderStats();renderEpisodes();drawDetail(S.events.find(e=>e.id===S.selected));
...
async function pollJob(){
  while(true){const job=await api('job');$('jobStatus').textContent=JSON.stringify(job,null,2);if(job.state==='error')throw Error(job.message);if(job.state==='done')return job;await new Promise(r=>setTimeout(r,700));}
}
async function importFiles(fileList){
  if(S.uploading)return;const selected=Array.from(fileList);if(!selected.length)return;
  S.uploading=true;$('files').disabled=true;$('fetchMarket').disabled=true;
  try{for(let i=0;i<selected.length;i++){const file=selected[i];if(file.size>512*1024*1024)throw Error('파일당 512MB 제한');$('jobStatus').textContent=file.name+' 로컬 전송 중…';$('jobProgress').value=i/selected.length*100;await api('import?name='+encodeURIComponent(file.name),{method:'POST',headers:{'Content-Type':'application/octet-stream'},body:file});await pollJob();}$('jobProgress').value=100;await refreshStatus();await loadEpisodes();if(!S.episode&&S.episodes.length)await selectEpisode(S.episodes.find(e=>e.id==='3086')||S.episodes[0]);else if(S.episode){const ep=S.episodes.find(e=>e.id===S.episode.id);if(ep)await selectEpisode(ep);}toast('데이터 가져오기 완료. 닫기를 눌러 차트를 확인하세요.');}catch(e){$('jobStatus').textContent='실패: '+e.message;error(e);}finally{S.uploading=false;$('files').disabled=false;$('fetchMarket').disabled=false;}
}
async function capture(){
  if(!S.bars.some(b=>b.open!=null))return toast('저장할 실제 캔들이 없습니다.');
  const source=chart.takeScreenshot(true,false),canvas=document.createElement('canvas');canvas.width=source.width;canvas.height=source.height+80;const ctx=canvas.getContext('2d');ctx.fillStyle='#fff';ctx.fillRect(0,0,canvas.width,canvas.height);ctx.fillStyle='#17263a';ctx.font='bold 15px sans-serif';ctx.fillText($('chartTitle').textContent+' · '+S.zone,18,26);ctx.drawImage(source,0,40);ctx.font='10px sans-serif';ctx.fillStyle='#7a8799';ctx.fillText('Binance spot proxy / BitMEX recorded orders / source time assumed UTC / research, not a trading signal',18,canvas.height-15);canvas.toBlob(blob=>{const url=URL.createObjectURL(blob);const a=node('a');a.href=url;a.download=`AOA_${S.episode.id}_${S.tf}.png`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);});
}
function bind(){
  ...
  $('replayEnabled').onchange=()=>{if($('replayEnabled').checked){$('endpoint').value='first';const idx=S.bars.findIndex(b=>b.time>=bucket(S.episode?.focus||S.start,S.tf));$('replaySlider').value=Math.max(0,idx-3);}applyReplay();};
  $('replaySlider').oninput=applyReplay;$('step').onclick=()=>{if(!$('replayEnabled').checked){$('replayEnabled').checked=true;applyReplay();}nextStep();};
  $('play').onclick=()=>{if(S.timer){clearInterval(S.timer);S.timer=null;$('play').textContent='▶ 재생';return;}if(!$('replayEnabled').checked){$('replayEnabled').checked=true;applyReplay();}S.timer=setInterval(nextStep,500);$('play').textContent='Ⅱ 일시정지';};
  ...
  $('export').onclick=()=>{if(S.episode){const a=node('a');a.href='/api/export?episode='+encodeURIComponent(S.episode.id);a.download='aoa_episode.csv';a.click();}};
  $('files').onchange=()=>importFiles($('files').files);
  $('dropzone').ondragover=e=>e.preventDefault();$('dropzone').ondrop=e=>{e.preventDefault();importFiles(e.dataTransfer.files);};
...
boot().catch(error);
// Minimal read-only diagnostics for automated browser tests; not an account integration.
