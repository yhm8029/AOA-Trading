...
async function refreshStatus(){const d=await api('status');S.token=d.token;$('dbStatus').textContent=`v${d.version} · 주문 ${d.orders.toLocaleString()}건 · 잔고 ${d.seed_daily_verified_days?'일별 '+d.seed_daily_verified_days+'일(참고)':d.seed_snapshots?d.seed_snapshots+'건':d.seed_data_connected?'검산 보류':'미연결'}`;$('empty').hidden=d.orders>0;$('versionWarning').hidden=d.version===UI_VERSION;if(d.version!==UI_VERSION)$('versionWarning').textContent=`앱 화면 ${UI_VERSION} / 실행 서버 ${d.version}: 버전이 다릅니다. 옛 실행 창을 종료하고 최신 폴더에서 다시 시작하세요.`;return d;}
...
function renderEpisodes(){
  ...
  for(const ep of S.episodes){const b=node('button',null,'episode-item'+(S.episode?.id===ep.id?' selected':''));b.dataset.episode=ep.id;const r=node('div',null,'row');r.append(node('strong','#'+ep.id),node('span',(ep.carried?'이월 · ':'')+(ep.direction||'미확보'),'badge '+(ep.direction||'').toLowerCase()));b.append(r,node('small',date(ep.start,true)+(S.cutoff==null?' · '+ep.count+'주문':'')));if(S.cutoff==null&&ep.pnl_btc!=null)b.append(node('small',`${ep.pnl_btc>0?'+':''}${ep.pnl_btc.toFixed(3)} BTC`,ep.pnl_btc>=0?'positive':'negative'));if(S.cutoff==null&&(ep.net_return_pct!=null||ep.net_return_estimate_pct!=null))b.append(node('small',returnText(ep)+(ep.net_return_pct==null?' · 참고':' · 순손익률'),'episode-return'));if(S.cutoff==null&&ep.seed_return_pct!=null)b.append(node('small',(ep.seed_basis==='prior_day_ledger_close'?'초기 전일잔고 ':'초기 시드 ')+seedReturnText(ep),'seed-return'));b.onclick=()=>selectEpisode(ep).catch(error);$('episodeList').append(b);}
}
